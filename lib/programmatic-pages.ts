export type ProgrammaticPage = {
  slug: string;
  title: string;
  description: string;
  category: string;
  relatedTool: string;
  parentLanding: string;
  indexable?: boolean;
};

export const programmaticPages: ProgrammaticPage[] = [
  {
    "slug": "jpg-to-png-online",
    "title": "JPG to PNG Online",
    "description": "Convert JPG to PNG online in your browser without installing extra software.",
    "category": "Image",
    "relatedTool": "jpg-to-png",
    "parentLanding": "/image-tools"
  },
  {
    "slug": "png-to-jpg-online",
    "title": "PNG to JPG Online",
    "description": "Convert PNG to JPG online for uploads, email, and websites.",
    "category": "Image",
    "relatedTool": "png-to-jpg",
    "parentLanding": "/image-tools"
  },
  {
    "slug": "compress-image-for-email",
    "title": "Compress Image for Email",
    "description": "Reduce image size before sending files by email.",
    "category": "Image",
    "relatedTool": "compress-image",
    "parentLanding": "/image-tools"
  },
  {
    "slug": "compress-image-for-website",
    "title": "Compress Image for Website",
    "description": "Shrink image size for faster pages and cleaner uploads.",
    "category": "Image",
    "relatedTool": "compress-image",
    "parentLanding": "/image-tools"
  },
  {
    "slug": "merge-pdf-online",
    "title": "Merge PDF Online",
    "description": "Combine PDF files online in a simple browser-based workflow.",
    "category": "PDF",
    "relatedTool": "merge-pdf",
    "parentLanding": "/pdf-tools"
  },
  {
    "slug": "split-pdf-online",
    "title": "Split PDF Online",
    "description": "Extract selected pages from a PDF online.",
    "category": "PDF",
    "relatedTool": "split-pdf",
    "parentLanding": "/pdf-tools"
  },
  {
    "slug": "loan-calculator-monthly-payment",
    "title": "Loan Calculator Monthly Payment",
    "description": "Estimate monthly loan payments using a simple online calculator.",
    "category": "Finance",
    "relatedTool": "loan-calculator",
    "parentLanding": "/online-calculators"
  },
  {
    "slug": "mortgage-calculator-payment",
    "title": "Mortgage Calculator Payment",
    "description": "Estimate mortgage payment amounts before you apply.",
    "category": "Finance",
    "relatedTool": "mortgage-calculator",
    "parentLanding": "/online-calculators"
  },
  {
    "slug": "bmi-calculator-for-men",
    "title": "BMI Calculator for Men",
    "description": "Estimate BMI quickly with a simple BMI calculator for men.",
    "category": "Health",
    "relatedTool": "bmi-calculator",
    "parentLanding": "/online-calculators"
  },
  {
    "slug": "bmi-calculator-for-women",
    "title": "BMI Calculator for Women",
    "description": "Estimate BMI quickly with a simple BMI calculator for women.",
    "category": "Health",
    "relatedTool": "bmi-calculator",
    "parentLanding": "/online-calculators"
  },
  {
    "slug": "tip-calculator-by-bill-amount",
    "title": "Tip Calculator by Bill Amount",
    "description": "Calculate tip and total by bill amount in a few seconds.",
    "category": "Utility",
    "relatedTool": "tip-calculator",
    "parentLanding": "/free-tools"
  },
  {
    "slug": "percentage-calculator-online",
    "title": "Percentage Calculator Online",
    "description": "Solve percentage questions online without doing the math by hand.",
    "category": "Utility",
    "relatedTool": "percentage-calculator",
    "parentLanding": "/free-tools"
  },
  {
    "slug": "json-formatter-online",
    "title": "JSON Formatter Online",
    "description": "Format and beautify JSON online in your browser.",
    "category": "Utility",
    "relatedTool": "json-formatter",
    "parentLanding": "/free-tools"
  },
  {
    "slug": "base64-encoder-online",
    "title": "Base64 Encoder Online",
    "description": "Encode text to Base64 online instantly.",
    "category": "Utility",
    "relatedTool": "base64-encoder",
    "parentLanding": "/free-tools"
  },
  {
    "slug": "base64-decoder-online",
    "title": "Base64 Decoder Online",
    "description": "Decode Base64 text online instantly.",
    "category": "Utility",
    "relatedTool": "base64-decoder",
    "parentLanding": "/free-tools"
  },
  {
    "slug": "url-encoder-online",
    "title": "URL Encoder Online",
    "description": "Encode text for URL use online.",
    "category": "Utility",
    "relatedTool": "url-encoder",
    "parentLanding": "/free-tools"
  },
  {
    "slug": "url-decoder-online",
    "title": "URL Decoder Online",
    "description": "Decode URL text online instantly.",
    "category": "Utility",
    "relatedTool": "url-decoder",
    "parentLanding": "/free-tools"
  },
  {
    "slug": "uuid-generator-online",
    "title": "UUID Generator Online",
    "description": "Generate UUIDs online in your browser.",
    "category": "Utility",
    "relatedTool": "uuid-generator",
    "parentLanding": "/free-tools"
  },
  {
    "slug": "slug-generator-online",
    "title": "Slug Generator Online",
    "description": "Generate clean URL slugs from text online.",
    "category": "Writing",
    "relatedTool": "slug-generator",
    "parentLanding": "/free-tools"
  },
  {
    "slug": "remove-line-breaks-online",
    "title": "Remove Line Breaks Online",
    "description": "Remove line breaks from text online.",
    "category": "Writing",
    "relatedTool": "remove-line-breaks",
    "parentLanding": "/free-tools"
  },
  {
    "slug": "text-sorter-online",
    "title": "Text Sorter Online",
    "description": "Sort lines of text alphabetically online.",
    "category": "Writing",
    "relatedTool": "text-sorter",
    "parentLanding": "/free-tools"
  },
  {
    "slug": "duplicate-line-remover-online",
    "title": "Duplicate Line Remover Online",
    "description": "Remove duplicate lines from text online.",
    "category": "Writing",
    "relatedTool": "duplicate-line-remover",
    "parentLanding": "/free-tools"
  },
  {
    "slug": "png-compressor-online",
    "title": "PNG Compressor Online",
    "description": "Compress PNG images online in your browser.",
    "category": "Image",
    "relatedTool": "png-compressor",
    "parentLanding": "/image-tools"
  },
  {
    "slug": "webp-to-png-online",
    "title": "WEBP to PNG Online",
    "description": "Convert WEBP images to PNG online.",
    "category": "Image",
    "relatedTool": "webp-to-png",
    "parentLanding": "/image-tools"
  },
  {
    "slug": "text-reverser-online",
    "title": "Text Reverser Online",
    "description": "Reverse text online instantly.",
    "category": "Writing",
    "relatedTool": "text-reverser",
    "parentLanding": "/free-tools"
  },
  {
    "slug": "remove-extra-spaces-online",
    "title": "Remove Extra Spaces Online",
    "description": "Remove extra spaces from text online.",
    "category": "Writing",
    "relatedTool": "remove-extra-spaces",
    "parentLanding": "/free-tools"
  },
  {
    "slug": "line-counter-online",
    "title": "Line Counter Online",
    "description": "Count lines in text online.",
    "category": "Writing",
    "relatedTool": "line-counter",
    "parentLanding": "/free-tools"
  },
  {
    "slug": "word-frequency-counter-online",
    "title": "Word Frequency Counter Online",
    "description": "Count the most common words in text online.",
    "category": "Writing",
    "relatedTool": "word-frequency-counter",
    "parentLanding": "/free-tools"
  },
  {
    "slug": "text-repeater-online",
    "title": "Text Repeater Online",
    "description": "Repeat text online quickly.",
    "category": "Writing",
    "relatedTool": "text-repeater",
    "parentLanding": "/free-tools"
  },
  {
    "slug": "text-to-binary-online",
    "title": "Text to Binary Online",
    "description": "Convert text to binary online.",
    "category": "Utility",
    "relatedTool": "text-to-binary",
    "parentLanding": "/free-tools"
  },
  {
    "slug": "binary-to-text-online",
    "title": "Binary to Text Online",
    "description": "Convert binary to text online.",
    "category": "Utility",
    "relatedTool": "binary-to-text",
    "parentLanding": "/free-tools"
  },
  {
    "slug": "rgb-to-hex-online",
    "title": "RGB to HEX Online",
    "description": "Convert RGB to HEX online.",
    "category": "Utility",
    "relatedTool": "rgb-to-hex",
    "parentLanding": "/free-tools"
  },
  {
    "slug": "hex-to-rgb-online",
    "title": "HEX to RGB Online",
    "description": "Convert HEX to RGB online.",
    "category": "Utility",
    "relatedTool": "hex-to-rgb",
    "parentLanding": "/free-tools"
  },
  {
    "slug": "unix-timestamp-converter-online",
    "title": "Unix Timestamp Converter Online",
    "description": "Convert Unix timestamps online.",
    "category": "Date and Time",
    "relatedTool": "unix-timestamp-converter",
    "parentLanding": "/online-calculators"
  },
  {
    "slug": "random-letter-generator-online",
    "title": "Random Letter Generator Online",
    "description": "Generate random letters online.",
    "category": "Utility",
    "relatedTool": "random-letter-generator",
    "parentLanding": "/free-tools"
  },
  {
    "slug": "password-strength-checker-online",
    "title": "Password Strength Checker Online",
    "description": "Check password strength online.",
    "category": "Utility",
    "relatedTool": "password-strength-checker",
    "parentLanding": "/free-tools"
  },
  {
    "slug": "uuid-bulk-generator-online",
    "title": "UUID Bulk Generator Online",
    "description": "Generate many UUIDs online.",
    "category": "Utility",
    "relatedTool": "uuid-bulk-generator",
    "parentLanding": "/free-tools"
  },
  {
    "slug": "meta-description-counter-online",
    "title": "Meta Description Counter Online",
    "description": "Count SEO meta description characters online.",
    "category": "Writing",
    "relatedTool": "meta-description-counter",
    "parentLanding": "/free-tools"
  },
  {
    "slug": "html-escape-online",
    "title": "HTML Escape Online",
    "description": "Escape HTML entities online.",
    "category": "Utility",
    "relatedTool": "html-escape",
    "parentLanding": "/free-tools"
  },
  {
    "slug": "html-unescape-online",
    "title": "HTML Unescape Online",
    "description": "Unescape HTML entities online.",
    "category": "Utility",
    "relatedTool": "html-unescape",
    "parentLanding": "/free-tools"
  },
  {
    "slug": "list-randomizer-online",
    "title": "List Randomizer Online",
    "description": "Shuffle lists into random order online.",
    "category": "Utility",
    "relatedTool": "list-randomizer",
    "parentLanding": "/free-tools"
  },
  {
    "slug": "roman-numeral-converter-online",
    "title": "Roman Numeral Converter Online",
    "description": "Convert numbers and Roman numerals online.",
    "category": "Utility",
    "relatedTool": "roman-numeral-converter",
    "parentLanding": "/free-tools"
  },
  {
    "slug": "how-to-use-a-sales-tax-calculator",
    "title": "How to Use a Sales Tax Calculator",
    "description": "Learn how to calculate sales tax, subtotal, and final total for quotes and purchases.",
    "category": "Business",
    "relatedTool": "sales-tax-calculator",
    "parentLanding": "/online-calculators",
    "indexable": true
  },
  {
    "slug": "credit-card-payoff-how-long-will-it-take",
    "title": "Credit Card Payoff: How Long Will It Take?",
    "description": "Estimate how many months it may take to pay off a credit card balance with fixed monthly payments.",
    "category": "Finance",
    "relatedTool": "credit-card-payoff-calculator",
    "parentLanding": "/online-calculators",
    "indexable": true
  },
  {
    "slug": "business-days-between-dates",
    "title": "Business Days Between Dates",
    "description": "Count weekdays between two dates for deadlines, shipping windows, and project planning.",
    "category": "Date and Time",
    "relatedTool": "business-days-calculator",
    "parentLanding": "/online-calculators",
    "indexable": true
  },
  {
    "slug": "weeks-between-two-dates",
    "title": "Weeks Between Two Dates",
    "description": "See how many full weeks and extra days fall between two dates.",
    "category": "Date and Time",
    "relatedTool": "weeks-between-dates",
    "parentLanding": "/online-calculators",
    "indexable": true
  },
  {
    "slug": "reading-level-checker-for-blog-posts",
    "title": "Reading Level Checker for Blog Posts",
    "description": "Review readability before you publish blog posts, landing pages, or emails.",
    "category": "Writing",
    "relatedTool": "reading-level-checker",
    "parentLanding": "/free-tools",
    "indexable": true
  },
  {
    "slug": "headline-title-case-checker",
    "title": "Headline Title Case Checker",
    "description": "Convert headlines to title case and review headline length before publishing.",
    "category": "Writing",
    "relatedTool": "title-case-headline-analyzer",
    "parentLanding": "/free-tools",
    "indexable": true
  },
  {
    "slug": "webp-to-jpg-online",
    "title": "WEBP to JPG Online",
    "description": "Convert WEBP images to JPG online for easier sharing, uploads, and editing.",
    "category": "Image",
    "relatedTool": "webp-to-jpg",
    "parentLanding": "/image-tools",
    "indexable": true
  },
  {
    "slug": "how-to-check-reading-level",
    "title": "How to Check Reading Level",
    "description": "See how readability tools estimate grade level and how to simplify text for everyday readers.",
    "category": "Writing",
    "relatedTool": "reading-level-checker",
    "parentLanding": "/free-tools",
    "indexable": true
  },
  {
    "slug": "sales-tax-before-and-after-price",
    "title": "Sales Tax Before and After Price",
    "description": "Work out the tax amount and final total from a pre-tax price in a few seconds.",
    "category": "Business",
    "relatedTool": "sales-tax-calculator",
    "parentLanding": "/online-calculators",
    "indexable": true
  },
  {
    "slug": "credit-card-interest-vs-payment",
    "title": "Credit Card Interest vs Payment",
    "description": "Compare monthly payment size to payoff time and interest cost on a revolving card balance.",
    "category": "Finance",
    "relatedTool": "credit-card-payoff-calculator",
    "parentLanding": "/online-calculators",
    "indexable": true
  },
  {
    "slug": "rotate-pdf-online",
    "title": "Rotate PDF Online",
    "description": "Rotate PDF pages online and download the updated document.",
    "category": "PDF",
    "relatedTool": "rotate-pdf",
    "parentLanding": "/pdf-tools",
    "indexable": true
  },
  {
    "slug": "delete-pdf-pages-online",
    "title": "Delete PDF Pages Online",
    "description": "Remove selected pages from a PDF online in your browser.",
    "category": "PDF",
    "relatedTool": "delete-pdf-pages",
    "parentLanding": "/pdf-tools",
    "indexable": true
  },
  {
    "slug": "reorder-pdf-pages-online",
    "title": "Reorder PDF Pages Online",
    "description": "Change the order of pages in a PDF online.",
    "category": "PDF",
    "relatedTool": "reorder-pdf-pages",
    "parentLanding": "/pdf-tools",
    "indexable": true
  },
  {
    "slug": "pdf-page-counter-online",
    "title": "PDF Page Counter Online",
    "description": "Count PDF pages online before you share or upload a file.",
    "category": "PDF",
    "relatedTool": "pdf-page-counter",
    "parentLanding": "/pdf-tools",
    "indexable": true
  },
  {
    "slug": "resize-image-to-kb",
    "title": "Resize Image to KB",
    "description": "Reduce an image toward a target KB size for forms, portals, and uploads.",
    "category": "Image",
    "relatedTool": "image-resizer-by-kb",
    "parentLanding": "/image-tools",
    "indexable": true
  },
  {
    "slug": "jpg-to-webp-online",
    "title": "JPG to WEBP Online",
    "description": "Convert JPG to WEBP online for smaller web-ready image files.",
    "category": "Image",
    "relatedTool": "jpg-to-webp",
    "parentLanding": "/image-tools",
    "indexable": true
  },
  {
    "slug": "png-to-webp-online",
    "title": "PNG to WEBP Online",
    "description": "Convert PNG to WEBP online in your browser.",
    "category": "Image",
    "relatedTool": "png-to-webp",
    "parentLanding": "/image-tools",
    "indexable": true
  },
  {
    "slug": "jpeg-to-jpg-online",
    "title": "JPEG to JPG Online",
    "description": "Convert JPEG to JPG online for easier uploads and compatibility.",
    "category": "Image",
    "relatedTool": "jpeg-to-jpg",
    "parentLanding": "/image-tools",
    "indexable": true
  },
  {
    "slug": "gif-to-jpg-online",
    "title": "GIF to JPG Online",
    "description": "Convert a GIF image to JPG online for simpler sharing.",
    "category": "Image",
    "relatedTool": "gif-to-jpg",
    "parentLanding": "/image-tools",
    "indexable": true
  },
  {
    "slug": "svg-to-png-online",
    "title": "SVG to PNG Online",
    "description": "Convert SVG files to PNG online for uploads and design handoffs.",
    "category": "Image",
    "relatedTool": "svg-to-png",
    "parentLanding": "/image-tools",
    "indexable": true
  },
  ,{
    "slug": "how-many-days-between-two-dates",
    "title": "How Many Days Between Two Dates",
    "description": "Count the calendar days between two dates for planning, contracts, travel, and deadlines.",
    "category": "Date and Time",
    "relatedTool": "days-between-dates",
    "parentLanding": "/online-calculators",
    "indexable": true
  }
  ,{
    "slug": "calculate-age-from-date-of-birth",
    "title": "Calculate Age From Date of Birth",
    "description": "Work out exact age from a date of birth in years, months, and days.",
    "category": "Date and Time",
    "relatedTool": "age-calculator",
    "parentLanding": "/online-calculators",
    "indexable": true
  }
  ,{
    "slug": "how-many-words-is-a-5-minute-speech",
    "title": "How Many Words Is a 5 Minute Speech",
    "description": "Estimate a five minute speech length with a quick word count workflow.",
    "category": "Writing",
    "relatedTool": "word-counter",
    "parentLanding": "/free-tools",
    "indexable": true
  }
  ,{
    "slug": "how-to-split-a-bill-with-tip",
    "title": "How to Split a Bill With Tip",
    "description": "Calculate tip, total, and per-person share when splitting a restaurant bill.",
    "category": "Utility",
    "relatedTool": "tip-calculator",
    "parentLanding": "/free-tools",
    "indexable": true
  }
  ,{
    "slug": "how-to-create-a-strong-password",
    "title": "How to Create a Strong Password",
    "description": "Create a stronger password for email, banking, work, and other important accounts.",
    "category": "Utility",
    "relatedTool": "password-generator",
    "parentLanding": "/free-tools",
    "indexable": true
  }
  ,{
    "slug": "jpg-vs-png-for-screenshots",
    "title": "JPG vs PNG for Screenshots",
    "description": "Compare JPG and PNG for screenshots, graphics, and common upload workflows.",
    "category": "Image",
    "relatedTool": "jpg-to-png",
    "parentLanding": "/image-tools",
    "indexable": true
  }
  ,{
    "slug": "png-to-jpg-for-email-and-web",
    "title": "PNG to JPG for Email and Web",
    "description": "Convert PNG to JPG when you need lighter image files for email and websites.",
    "category": "Image",
    "relatedTool": "png-to-jpg",
    "parentLanding": "/image-tools",
    "indexable": true
  }
  ,{
    "slug": "remove-pages-from-a-pdf",
    "title": "How to Remove Pages From a PDF",
    "description": "Delete unwanted pages from a PDF before sending, uploading, or archiving the file.",
    "category": "PDF",
    "relatedTool": "delete-pdf-pages",
    "parentLanding": "/pdf-tools",
    "indexable": true
  }
  ,{
    "slug": "how-to-merge-pdf-files-for-upload",
    "title": "How to Merge PDF Files for Upload",
    "description": "Combine several PDF files into one clean document for portals, school, or work uploads.",
    "category": "PDF",
    "relatedTool": "merge-pdf",
    "parentLanding": "/pdf-tools",
    "indexable": true
  }
  ,{
    "slug": "extract-pages-from-a-pdf",
    "title": "How to Extract Pages From a PDF",
    "description": "Split out the pages you need from a larger PDF before sharing the file.",
    "category": "PDF",
    "relatedTool": "split-pdf",
    "parentLanding": "/pdf-tools",
    "indexable": true
  }
  ,{
    "slug": "how-to-calculate-a-loan-payment",
    "title": "How to Calculate a Loan Payment",
    "description": "Estimate a monthly loan payment based on amount, interest rate, and term.",
    "category": "Finance",
    "relatedTool": "loan-calculator",
    "parentLanding": "/online-calculators",
    "indexable": true
  }
  ,{
    "slug": "how-to-calculate-a-mortgage-payment",
    "title": "How to Calculate a Mortgage Payment",
    "description": "Estimate a monthly mortgage payment before comparing homes or loan offers.",
    "category": "Finance",
    "relatedTool": "mortgage-calculator",
    "parentLanding": "/online-calculators",
    "indexable": true
  }
  ,{
    "slug": "bmi-calculator-for-adults",
    "title": "BMI Calculator for Adults",
    "description": "Use a BMI calculator to estimate adult body mass index quickly.",
    "category": "Health",
    "relatedTool": "bmi-calculator",
    "parentLanding": "/online-calculators",
    "indexable": true
  }
  ,{
    "slug": "how-many-business-days-between-dates",
    "title": "How Many Business Days Between Dates",
    "description": "Count weekdays between two dates for work deadlines, shipping windows, and schedules.",
    "category": "Date and Time",
    "relatedTool": "business-days-calculator",
    "parentLanding": "/online-calculators",
    "indexable": true
  }
  ,{
    "slug": "how-to-pay-off-credit-card-debt-faster",
    "title": "How to Pay Off Credit Card Debt Faster",
    "description": "Compare payment strategies and payoff timelines for credit card balances.",
    "category": "Finance",
    "relatedTool": "credit-card-payoff-calculator",
    "parentLanding": "/online-calculators",
    "indexable": true
  }
  ,{
    "slug": "resize-image-to-100kb",
    "title": "Resize Image to 100KB",
    "description": "Reduce an image toward 100KB for strict upload forms and applications.",
    "category": "Image",
    "relatedTool": "image-resizer-by-kb",
    "parentLanding": "/image-tools",
    "indexable": true
  }

];
